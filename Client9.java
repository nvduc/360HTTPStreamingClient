...
public class Client9 {
	...
	public static double[] buffer_time_seg = new double[200];
	public static double[] buffer_size_seg = new double[200];
	/* socket */
...
//			N_seg_tmp = 200 - k * 56;
    ...
    for(int seg_id = 0; seg_id < 180; seg_id++) {
    	System.out.println("----------------------------------------------------------");
    	System.out.println("segment: "+ seg_id);
    	
    	seg_id_real = seg_id % N_seg; // seg_id % 56: 0->0;55->55; 56->0
	    ...
	    phi = head_trace[seg_id_real * SEG_LEN][0];
	    theta = head_trace[seg_id_real * SEG_LEN][1];
	  ...
	  //in ra tile_weight
	    ...
	    sum_invisible_br = 0;
	    for(tile_id = 0; tile_id < invisible_tile_list.size(); tile_id ++) {
			sum_invisible_br += tile_BR[seg_id_real][ invisible_tile_list.get(tile_id)][0]; 
	    }	
	    ...
	    for(version_id = 0; version_id < N_version; version_id++) {
	        // tinh tong bitrate cua tat ca cac visible_tile
	        	...
	        	for(tile_id = 0; tile_id < visible_tile_list.size(); tile_id ++)
	        		sum_visible_br += tile_BR[seg_id_real][visible_tile_list.get(tile_id)][version_id]; 
	...
	//      }	
	    ...
	    for(version_id = 0; version_id < N_version; version_id++) {
	    	...
	    	sum_br = 0;
	    	for(tile_id = 0; tile_id < N_tile; tile_id ++)
	    		sum_br += tile_BR[seg_id_real][tile_id][version_id];
	    	if(sum_br > RC)
	    		break;
	    }
				...
				if(visible_tile_rear_list.size() > 0 && tile_version[visible_tile_rear_list.get(0)] != 0 ) {
					...
					for (ver = tile_version[visible_tile_rear_list.get(0)]; ver > -1; ver--) {
						...
						for (int tile_rear_id = tile_id+1; tile_rear_id < N_tile; tile_rear_id++) {
							sum_rear_br += tile_BR[seg_id_real][tile_rear_id][tile_version[tile_rear_id]];
						}
	//					System.out.println(sum_rear_br);
	    	...
	    	tile_size = tile_BR[seg_id_real][tile_id][tile_version[tile_id]]; // kbits = Bitrate x do dai (1)
	    	sum_tile_BR+=tile_size;
	    	i=0;
	    	while(t_sum > time_trace[i+0]) i++;
	    	if((time_trace[i+0] - t_sum) * bandwidth_trace[i+0] > tile_size) {
	    		tile_download_time = tile_size/bandwidth_trace[i+0];
	    		System.out.println("mark 1");
	    	}	
	    	else {
	    		tile_download_time = (time_trace[i+0] - t_sum) + (tile_size - (time_trace[i+0]-t_sum)*bandwidth_trace[i+0])/bandwidth_trace[i+0+1];
	    		System.out.println("mark 2 ");
	    	}
	    	System.out.println("tile_size "+tile_size);
	    	System.out.println("tile_download_time "+tile_download_time);
...
System.out.println("t_sum "+t_sum);
	    }
	    ...
	    for(tile_id = 0; tile_id < N_tile; tile_id ++) {
	    	vp_quality += tile_PSNR[seg_id_real][tile_id][tile_version[tile_id]] * tile_weight[tile_id]; // weighted sum of tile quality
	    }
...
//		    	buffer_time_seg[seg_id]=0;
		    ...
		    if(t_sum < 0 + (seg_id+1) * 1.0) {
		    	delay_time = (0 + (seg_id + 1) * 1.0) - t_sum;
		    	t_sum = 0 + (seg_id + 1) * 1.0;
		    }else {
		    	delay_time = 0;
		    }
